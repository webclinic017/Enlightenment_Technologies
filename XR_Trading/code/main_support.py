...
def run_model(prediction_parameters, training_data, testing_data):
    ...
    # Process Data
    all_data, target, train_data, test_data, training_data_len, sc, so2_scale, num_data = data_preprocessing(training_set_len, prediction_window, training_data, testing_data)
    ...
    for i in range(0, prediction_num): 
        ...
        predicted_so2 = forcast(model, X_test, so2_scale, prediction_window)
    ...
    # Visualize Results
    visualize_fit(all_data, training_data_len, multiple_predictions, prediction_num, model_label, plot_other_predictions)
